...
def main():
    rospy.init_node('yellowglobal_planner_node', anonymous=False)
...
#     rospy.Subscriber("/obst_4", Pose2D, callback_obs_4)
    ...
    bluebot = apf("/blue")
    redbot = apf("/red")
    yellowbot = apf("/yellow","/formation_YELLOW","/X_YELLOW","/Y_YELLOW","/THETA_YELLOW","/replan_YELLOW")
    purplebot = apf("/purple")
    time.sleep(1)

    while not rospy.is_shutdown():   
        if yellowbot.getReplanFlagVar() >= 1:
            yellowbot.plan(redbot.getPose(),bluebot.getPose(),purplebot.getPose())
            # yellowbot.setReplanFlagVar(0)
            yellowbot.updatePath()
            print "yellow Path is out!!!"
